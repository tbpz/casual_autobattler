...
> **RETIRED 2026-08-15 — history only, not current.** Superseded end to end by the 2026-08-04 legibility rewrite, the 2026-08-06/07/09 fight-causality passes, and the 2026-08-15 chain rebuild (all in `../DECISIONS.md`): the two aggregate meters (§2), the flat bonus-hit table (§3), and the two-stage PRD ignition gate (§4) are all gone from the running build. The beat-sheet *shape* (§1) survives, already folded into `../STATE.md`'s design spine. Kept here, not deleted, because `../DECISIONS.md` cites this file's §3/§5 as grep targets and is append-only.
...
>
> **Further superseded (2026-08-06):** §1's "jeopardy is mandatory every fight" (the 0:08–0:16 dip row) is reversed — see DECISIONS.md's 2026-08-06 "in-fight jeopardy is no longer mandatory" entry. Whether a fight has a dip/ignition/chain at all now depends on the squad's composition (the same day's "squad pick is the risk dial" entry), and the ignition beat's screen-shake/callout/glow tell (§1's "Ignition" row) now only fires once a chain reaches length 3+ (the same day's "spectacle gated on payoff" entry) — a length-1 or length-2 chain gets a smaller, quieter tell. The beat sheet's overall *shape* (opening exchange → dip → ignition → chain → resolve) still describes what happens *when* a fight has jeopardy at all; it no longer describes every fight.
