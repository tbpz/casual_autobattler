...
> **RETIRED 2026-08-15 — history only, not current.** Fully consumed: all 30 questions are answered, either logged to `../DECISIONS.md` directly or settled by the build itself (Part 3's "stub it, let the build decide" questions — targeting, who goes hot, enemy shape, fight-end/between-fight detail — were all resolved by later passes; see `../DECISIONS.md`). Only Q27 ("how long before it gets boring") is still open, and `../CHAIN_AXIS_PLAN.md` now owns it. Kept here, not deleted, for history.
