...
> **Last synced:** 2026-07-19 (co-founder alignment resync)
...
2. **Fun probes** (not yet started — current step) — small, disposable, ugly toys that isolate the open knob in the shared moment (RNG vs. emergent combination as the surprise source), played and judged only by "do I not want to stop?"
...
- **"Couldn't fully predict"** — the open knob (see below): source of unpredictability is RNG or emergent combination, not yet resolved.
...
**Open knob / active probe question:** is the "couldn't fully predict" surprise generated by **RNG** (friend's stated favorite — bash/crit/non-targeted-skill procs) or **emergent combination** (Tu's stated favorite — deterministic systems colliding into unplanned behavior)? Bridge hypothesis to probe: **RNG triggers legible chains** — the dice decides *when/whether* a cascade fires, the player's setup decides *what it becomes*.
...
2. **Fun probes (current step, not yet started).** Build small disposable toys that isolate the shared moment's open knob and restore what the flat prototype throttled: watch-only combat with juice/variance restored, real stakes (no free same-difficulty retry), and a synergy system deep enough to test "run away past what I planned." Specifically probe the **RNG-triggers-legible-chains** bridge hypothesis (does a dice-triggered cascade satisfy both the RNG-lover and the emergence-lover?). Judge each probe by lean-in ("do I not want to stop?"), not argument.
